...
export CC_BACKEND_SERVERS="172.20.0.1"
...
docker run --name frontend --env CC_BACKEND_SERVERS=$CC_BACKEND_SERVERS -v /etc:/hypervisor_etc -p 8080:80 mrow4a/frontend:latest
docker ps -a
curl localhost:8080
curl localhost:8080
curl localhost:8080
